...
#import "NSString+JYB.h"
#import <CommonCrypto/CommonDigest.h>
#import <YYCategories/YYCategories.h>

@implementation NSString (JYB)

+ (NSString *)customerServiceUrl {
    return @"https://cschat-ccs.aliyun.com/index.htm?tntInstId=_1KIG6t6&scene=SCE00004604&hideShareBtn=1&hideCloseBtn=1";
}
...
- (NSString*) valueForSection:(NSString*)sec key:(NSString*)key{
    ...
    NSRange rngLeft = NSMakeRange(0, len);
    
    NSString *fSec = [NSString stringWithFormat:@"[%@]", sec];
    NSRange rngSection;
    while (YES) {
        
        rngSection = [self rangeOfString:fSec options:NSLiteralSearch range:rngLeft];
        if (rngSection.location == NSNotFound) {
            return @"";
        }
        if (rngSection.location+rngSection.length == len) {
            return @"";
        }
        if (rngSection.location == 0) {
            break;
        }
        if ([self characterAtIndex:rngSection.location-1] == '\n') {
            break;
        }
        
        rngLeft.location = rngSection.location+1;
        rngLeft.length = len-rngLeft.location;
    }
    
    rngLeft.location = rngSection.location+1;
    rngLeft.length = len-rngLeft.location;
    
    fSec = [NSString stringWithFormat:@"\n["];
    rngSection = [self rangeOfString:fSec options:NSLiteralSearch range:rngLeft];
    
    NSString *fKey = [NSString stringWithFormat:@"\n%@", key];
    NSRange rngKey;
    while (YES) {
        
        rngKey = [self rangeOfString:fKey options:NSLiteralSearch range:rngLeft];
        if (rngKey.location == NSNotFound) {
            return @"";
        }
        if (rngKey.location+rngKey.length >= len) {
            return @"";
        }
        
        if (rngSection.location != NSNotFound && rngKey.location > rngSection.location) {
            return @"";
        }
        
        NSInteger pos = rngKey.location+rngKey.length;
        BOOL findEq = NO;
        while (pos < len) {
        ...
        if (findEq) {
            rngLeft.location = pos;
            rngLeft.length = len-rngLeft.location;
            break;
        }
        
        rngLeft.location = rngKey.location+1;
        rngLeft.length = len-rngLeft.location;
    }
    
    NSString *val = @"";
    BOOL hasQuote = NO;
    NSInteger posLeft = rngLeft.location+1;
    while (posLeft < len) {
        
        unichar c = [self characterAtIndex:posLeft];
        if (c == ' ' || c == '\t') {
        ...
        else if ((c == '\r' && [self characterAtIndex:posLeft+1]=='\n') || c =='\0'){
            return @"";
        }
        else {
            break;
    ...
    NSInteger posRight = posLeft+1;
    while (posRight < len) {
        
        unichar c = [self characterAtIndex:posRight];
        if (c == '\"'){
            if (hasQuote) {
                posRight -= 1;
                break;
        ...
        else if ((c == '\r' && [self characterAtIndex:posRight+1]=='\n') || c =='\0'){
            posRight -= 1;
            break;
        }
    ...
    val = [self substringWithRange:NSMakeRange(posLeft, posRight-posLeft+1)];
    
    
    // .ini文件的\n前居然有多个\r，剔除掉多余的\r --add by weber
    if (val == nil) {
        val = @"";
    }else{
        NSRange range = [val rangeOfCharacterFromSet:[NSCharacterSet characterSetWithCharactersInString:@"\r"]];
